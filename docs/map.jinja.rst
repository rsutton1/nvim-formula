...
We will see a quick setup to configure the ``nvim`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``nvim`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``nvim`` formulas under ``/srv/salt/nvim/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/nvim/parameters/dns:domain/
    ln -s dns:domain /srv/salt/nvim/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/nvim/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/nvim/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``nvim`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``nvim`` formulas under ``/srv/salt/nvim/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/nvim/parameters/roles
...
We create a configuration for the role ``nvim/server`` in ``/srv/salt/nvim/parameters/roles/nvim/server.yaml``:
...
We create another configuration for the role ``nvim/client`` in ``/srv/salt/nvim/parameters/roles/nvim/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles nvim/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles nvim/client
...
.. code-block:: console
    ...
    mv /srv/salt/nvim/parameters/dns:domain/  '/srv/salt/nvim/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-nvim-as-json:
      test.nop:
        - name: {{ nvim | json }}
...
Here is an example based on `template-formula/nvim/config/file.sls`_:
...
This ``sls`` file expose a ``nvim`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ nvim['winner'] }}
...
.. _template-formula/nvim/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/nvim/config/file.sls
